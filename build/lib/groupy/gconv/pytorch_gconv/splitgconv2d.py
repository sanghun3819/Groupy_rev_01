...
class SplitGConv2D_SFF(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bias=True, input_stabilizer_size=1, output_stabilizer_size=4):
        super(SplitGConv2D_SFF, self).__init__()
        assert (input_stabilizer_size, output_stabilizer_size) in make_indices_functions.keys()
        self.ksize = kernel_size
...
class SplitGConv2D_SCC(nn.Module):
    ...
    def forward(self, input):
        tw = trans_filter(self.weight, self.inds) #64 8 64 8 3 3
        tw_scc_1 = trans_filter(self.weight1, self.inds) #64 8 64 8 3 3
        tw_scc_2 = trans_filter(self.weight2, self.inds)  # 64 8 64 8 3 3
        ...
        tw = tw.view(tw_shape) #512 512 3 3
        tw_scc_1 = tw_scc_1.view(tw_shape)
        tw_scc_2 = tw_scc_2.view(tw_shape)
...
class SplitGConv2D(nn.Module):
    ...
    def forward(self, input):
        ...
        input = input.view(input_shape[0], self.in_channels*self.input_stabilizer_size, input_shape[-2], input_shape[-1])
        #batch 512 w h
        #print('pz2p4m input size :', input.size())
        y1 = F.conv2d(input, weight=tw, bias=None, stride=self.stride,
                        padding=self.padding, dilation=1)
        #print('y1', y1.shape)
        batch_size, _, ny_out, nx_out = y1.size()
        y1 = y1.view(batch_size, self.out_channels, self.output_stabilizer_size, ny_out, nx_out)
        #print('y1', y1.shape)
        #print('pz2p4m output size :', y1.size())
...
class SplitGConv2D_EXPAND(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bias=True, input_stabilizer_size=1, output_stabilizer_size=4):
        super(SplitGConv2D_EXPAND, self).__init__()
        assert (input_stabilizer_size, output_stabilizer_size) in make_indices_functions.keys()
        self.ksize = kernel_size
    ...
    def forward(self, input):
        ...
        input_tot = input_tot.view(input_shape[0], self.in_channels * self.input_stabilizer_size*2, input_shape[-2],
                           input_shape[-1])
        # batch 1024 w h
        y1 = F.conv2d(input_tot, weight=tw_tot, bias=None, stride=self.stride,
                      padding=self.padding, dilation=1)
        # print('y1', y1.shape)
        batch_size, _, ny_out, nx_out = y1.size()
        y1 = y1.view(batch_size, self.out_channels, self.output_stabilizer_size, ny_out, nx_out)
...
class SplitGConv2D_EXPANSION(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bias=True, input_stabilizer_size=1, output_stabilizer_size=4):
        super(SplitGConv2D_EXPANSION, self).__init__()
        assert (input_stabilizer_size, output_stabilizer_size) in make_indices_functions.keys()
        self.ksize = kernel_size
    ...
    def forward(self, input):
        ...
        y2 = F.conv2d(input, weight=tw_expand, bias=None, stride=self.stride,
                      padding=self.padding, dilation=1)
        # print('y1', y1.shape)
...
class SplitGConv2D_EXPANSIONS(nn.Module):

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, bias=True, input_stabilizer_size=1, output_stabilizer_size=4):
        super(SplitGConv2D_EXPANSIONS, self).__init__()
        assert (input_stabilizer_size, output_stabilizer_size) in make_indices_functions.keys()
        self.ksize = kernel_size
    ...
    def forward(self, input):
        ...
        #print('tw expand size : ', tw_expand.size()) #512 512 3 3
        tw_tot = torch.cat([tw, tw_expand], dim=1)
        tw_tot1 = torch.cat([tw_expand, tw], dim=1)
        #print('tw tot size : ', tw_tot.size()) # 512 1024 3 3
...
class P8MConvZ2(SplitGConv2D_EXPANSION):

    def __init__(self, *args, **kwargs):
        super(P8MConvZ2, self).__init__(input_stabilizer_size=1, output_stabilizer_size=8, *args, **kwargs)

class P8MConvP8M(SplitGConv2D_EXPANSIONS):
...
class P4MConvP4M_EXPAND(SplitGConv2D_EXPAND):

    def __init__(self, *args, **kwargs):
        super(P4MConvP4M_EXPAND, self).__init__(input_stabilizer_size=8, output_stabilizer_size=8, *args, **kwargs)

class P4MConvP4M_SFF(SplitGConv2D_SFF):

    def __init__(self, *args, **kwargs):
        super(P4MConvP4M_SFF, self).__init__(input_stabilizer_size=8, output_stabilizer_size=8, *args, **kwargs)
